...
export class ChangePasswordPage {
  ...
  constructor(
    ...
    // securityCode: ["",[Validators.required, Validators.minLength(6), Validators.maxLength(6)]], // 验证码
      ...
      loginObj => {
        if (!_.isNull(loginObj) && !_.isEmpty(loginObj)) {
          if (
            ...
            const baseInfo: any = {};
            baseInfo.id = loginObj["UserInfo"]["id"]; // 拉包工信息ID
            GlobalMethod.setForm(this.formData, baseInfo); // 重新设置表单
          } else {
