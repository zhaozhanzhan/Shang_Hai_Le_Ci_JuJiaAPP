...
@IonicPage()
@Component({
  selector: "page-scan-two",
  templateUrl: "scan-two.html"
})
export class ScanTwoPage {
  @ViewChild(Content)
  content: Content;

  public scanType: string = null; // 扫描包裹类型 big大包，small小包
  ...
  constructor(
    ...
    this.scanType = this.navParams.get("scanType");
  }
      ...
      .then((status: QRScannerStatus) => {
        if (status.authorized) {
          let scanSub = this.qrScanner.scan().subscribe((text: string) => {
            if (this.scanType == "small") {
              // 小包
              if (/(^[Y]\d{9}$)/gi.test(text)) {
                console.error("正则匹配成功", text);
                ...
                this.httpReq.post(
                  "workerUser/getLittlePackageMessage",
                  null,
                  formData,
                  data => {
                    if (data["status"] == 200) {
                      if (data["code"] == 0) {
                        ...
                        if (_.isFunction(backRefreshObj.backRefEvent)) {
                          ...
                          backRefreshObj.backRefEvent(
                            backRefreshObj.that,
                            this.jsUtil.deepClone(data["data"])
                          ); // 执行返回刷新事件
                        }
                        // loading.dismiss();
                ...
                // this.gloService.showMsg("正则匹配成功", null, 3000);
              } else {
            ...
            } else if (this.scanType == "big") {
              // 大包
              if (/(^\d{2}[Q]\d{7}$)/gi.test(text)) {
                  ...
                  buttons: [
                    ...
                    },
                    {
                      ...
                      handler: () => {
                        ...
                        if (
                          _.isFunction(backRefreshObj.backAddBigPackageNoEvent)
                        ) {
                          console.error("执行返回刷新事件");
                          backRefreshObj.backAddBigPackageNoEvent(
                            backRefreshObj.that,
                            {
                              bigPackageNo: text
                            }
