...
export class TransListPage {
  ...
  public reqUrl: string = "bankContent/transactionDetail"; // 请求数据URL
      ...
      if (!_.isNull(loginObj) && !_.isEmpty(loginObj)) {
        ...
        if (
          ...
          if (
            ...
            this.sendData.customerId = userId; // 拉包工信息ID
            this.sendData.type = 1; // 区分来源(1:拉包工；2：商户)
            this.sendData.dateStr = this.jsUtil.dateFormat(new Date()); // 日期
            // 请求列表数据
            this.reqData(
              this.reqUrl,
              this.sendData,
      ...
      if (data["status"] == 200) {
        if (data["code"] == 0) {
          ...
          for (let i = 0; i < recData.length; i++) {
            for (const key in recData[i]) {
              if (recData[i].hasOwnProperty(key)) {
                if (key == "amount") {
                  const money: number = parseInt(recData[i][key]);
                  recData[i][key] = (money / 100).toFixed(2);
                }
                if (key == "transTime") {
                  let dateTime: string = recData[i][key];
                  let dateTimeArr: any = dateTime
                    .replace(/(.{2})/g, "$1 ")
                    .split(" ");
                  dateTime =
                    dateTimeArr[0] +
                    ...
                    ":" +
                    dateTimeArr[6];
                  recData[i][key] = dateTime;
                  // let date: string = dateTime.substring(0, 8);
                  // let time: string = dateTime.substring(8);
                  // console.error(date, time);
                  // console.error(dateTimeArr, dateTime);
                }
              }
            }
          }
   ...
   * @param {string} url 请求数据接口URL地址
   * @param {*} reqObj 请求数据参数对象
   * @memberof TransListPage
   */
  public search(url: string, reqObj: any) {
    reqObj.page = pageObj.currentPage; //重置当前页码
    reqObj.size = pageObj.everyItem; //重置当前页面请求条数
    console.error("下拉刷新执行");
    this.reqData(
