...
export class PackageListPage {
  ...
  public reqUrl: string = "workerUser/getTwoList"; // 请求数据URL
  ...
  public segArr = ["big", "small"]; // 顶部tab切换值
  public segTitObj = {
    // 顶部标题名称
    big: "大包列表",
    small: "小包列表"
      ...
      if (!_.isNull(loginObj) && !_.isEmpty(loginObj)) {
        ...
        if (
          ...
          if (
            ...
            // {"workerId": "拉包工Id","type": "(1:大包列表，2：小包列表)","page": "页码","size": "每页条数"}
            for (let i = 0; i < this.segArr.length; i++) {
              // 初始化每个页面对象独立
              ...
              this.pageArr[i]["sendData"]["workerId"] = userId; // 拉包工信息ID
              this.pageArr[i]["sendData"]["type"] = i + 1; //（1:大包列表，2：小包列表）
              // 请求列表数据
              this.reqData(
                this.reqUrl,
  ...
  public jumpPage(pageName: any, obj?: any, opts?: any): void {
    ParamService.setParamObj({
      ...
      backRefreshAllEvent: this.backRefreshAll
    }); // 保存返回刷新事件
            ...
            this.httpReq.post(
              ...
              data => {
                if (data["status"] == 200) {
                  if (data["code"] == 0) {
                    this.pageArr[1]["dataList"].splice(index, 1); // 删除当前操作元素
                    this.gloService.showMsg("取消成功！", null, 3000);
                  } else {
                    this.gloService.showMsg(data["message"], null, 3000);
                  }
                }
   ...
   * @memberof MyTaskPage
   */
  public backRefreshAll(that: any) {
    let userId: any = null;
    that.ionicStorage.get("loginInfo").then(loginObj => {
      console.error("loginInfo", loginObj);
      if (!_.isNull(loginObj) && !_.isEmpty(loginObj)) {
        // 判断是否是空对象
        if (
          ...
          if (
            ...
            for (let i = 0; i < that.segArr.length; i++) {
              // 初始化每个页面对象独立
              that.pageArr[i]["sendData"]["page"] = pageObj.currentPage; // 定义当前页码
              that.pageArr[i]["sendData"]["size"] = pageObj.everyItem; // 定义当前页面请求条数
              that.pageArr[i]["sendData"]["totalPage"] = pageObj.totalPage; // 定义当前页面请求条数
              // {"id": "当前拉包工Id","status": "（0：待处理，1：进行中，2：已完成）","page": "页码","size": "每页条数"}
              that.pageArr[i]["sendData"]["id"] = userId; // 拉包工信息ID
              that.pageArr[i]["sendData"]["status"] = i; // 人员属于（0：待处理，1：进行中，2：已完成，3：已超时）
              // 请求列表数据
              that.reqData(
                that.reqUrl,
                that.pageArr[i]["sendData"],
                res => {
                  // 请求数据成功
                  that.pageArr[i]["dataList"] = []; // 刷新成功清空列表
                  that.pageArr[i]["dataList"] = that.pageArr[i][
                    "dataList"
                  ].concat(res);
                  console.error(
                    "this.pageArr[i]['dataList']",
                    that.pageArr[i]["dataList"]
                  );
                },
                err => {
                  // 请求数据失败
