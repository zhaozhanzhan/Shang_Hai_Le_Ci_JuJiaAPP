...
@IonicPage()
@Component({
  selector: "page-roll-out",
  templateUrl: "roll-out.html"
})
export class RollOutPage {
  @ViewChild(Content)
  content: Content;

  public userId: any = null; // 表单信息
  public formInfo: any = {}; // 表单信息
  public formData: any; // 表单对象
  public serviceCharge: any = 0; // 转出手续费
  ...
  constructor(
    ...
    this.formData = this.fb.group({
      ...
      remark: ["", [Validators.maxLength(50)]] // 出金备注
    });
  }
   ...
   */
  public calSerCharge() {
    // {"customerId": "主键","type": "区分来源(1:拉包工；2：商户)","money":"出金金额"}
    const formData = this.jsUtil.deepClone(this.formData.value); // 深度拷贝表单数据
    let money = parseFloat(formData.money).toFixed(2);
    const reqData: any = {};
    reqData.customerId = this.userId; // 用户ID
    reqData.type = 1; // "区分来源(1:拉包工；2：商户)"
    reqData.money = parseFloat(money).toFixed(2); // 出金金额
    this.httpReq.post("bankContent/serviceCharge", null, formData, data => {
      if (data["status"] == 200) {
        if (data["code"] == 0) {
          this.serviceCharge = data["data"];
      ...
      buttons: [
        ...
        },
        {
          ...
          handler: () => {
            ...
            this.httpReq.post(
              "bankContent/capitalOut",
              null,
              formData,
              data => {
                if (data["status"] == 200) {
                  if (data["code"] == 0) {
                    this.gloService.showMsg("转出成功", null, 3000);
                    loading.dismiss();
                    this.navCtrl.pop();
                  } else {
                    this.gloService.showMsg(data["message"], null, 3000);
