...
@IonicPage()
@Component({
  selector: "page-stati-summary",
  templateUrl: "stati-summary.html"
})
export class StatiSummaryPage {
  @ViewChild(Content)
  content: Content;

  public reqUrl: string = "workerUser/totalForWorker"; // 请求数据URL
  public sendData: any = {}; // 定义请求数据时的对象
  public dataList: Array<any> = []; // 数据列表
  public totalObj: any = {}; // 统计数据对象
  ...
  constructor(
    ...
    // console.error(new OrderStatePipe().transform("0"));
  }

  ionViewDidLoad() {
    console.log("ionViewDidLoad StatiSummaryPage");
    ...
    this.sendData.startDate = ""; // 定义当前页面开始日期
    this.sendData.endDate = ""; // 定义当前页面结束日期
            ...
            this.reqData(
              ...
              res => {
                ...
                if (this.dataList.length == 0) {
                  this.gloService.showMsg("今天没有订单信息！", null, 2000);
                }
              ...
              err => {
                ...
                this.gloService.showMsg("请求数据失败", null, 2000);
              }
            );
          } else {
    ...
    this.httpReq.post(url, null, reqObj, data => {
      if (data["status"] == 200) {
        if (data["code"] == 0) {
          ...
          this.totalObj = this.jsUtil.deepClone(data["data"]["objectMap"]);
          suc(data["data"]["list"]);
  ...
  public downRefresh(ev: Refresher, url: string, reqObj: any) {
    reqObj.page = pageObj.currentPage; //重置当前页码
    reqObj.size = pageObj.everyItem; //重置当前页面请求条数
    reqObj.startDate = ""; // 重置开始日期
    reqObj.endDate = ""; // 重置结束日期
