...
export class WaybillListPage {
  ...
  public reqUrl: string = "workerUser/getClientListForWorker"; // 请求数据URL
      ...
      if (!_.isNull(loginObj) && !_.isEmpty(loginObj)) {
        ...
        if (
          ...
          if (
            ...
            this.sendData.workerId = userId; // 拉包工信息ID
            // 请求列表数据
            this.reqData(
              this.reqUrl,
              this.sendData,
  ...
  public delFun(id: any, arr: any, index: number) {
    ...
    this.httpReq.post("workerUser/getClientListForDel", null, reqObj, data => {
      if (data["status"] == 200) {
        if (data["code"] == 0) {
